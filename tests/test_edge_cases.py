...
@pytest.mark.parametrize("member", [None, "danielinbar"])
def test_invalid_member_edge_cases(member):
    with pytest.raises(ValueError):
        validate_member(member)
#1.2 test to check valid_members() should pass for valid categories with spaces " daniel "
@pytest.mark.edge_cases
def test_valid_members_with_spaces_accepted():
    assert validate_member(" daniel ") == "daniel"

#2.test to check is_valid_category() raises ValueError for invalid category edge cases - None and combined category names
@pytest.mark.edge_cases
@pytest.mark.parametrize("category", [None, "carbills"])
def test_invalid_category_edge_cases(category):
    with pytest.raises(ValueError):
        is_valid_category(category)

#2.2 test to check is_valid_category() should pass for valid categories with spaces  eg: " groceries "
@pytest.mark.edge_cases
def test_valid_category_with_spaces_accepted():
    assert is_valid_category(" groceries ") == True
